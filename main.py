...
def get_contributors_for_file(directory, filepath, extensions, weeks):
    ...
    command = "git -C {directory} blame {since}{filepath}".format(
        directory=directory, filepath=filepath, since=since)
    ...
    for line in all_lines:
        ...
        if line and line[0] == "^":
            continue  # any line before the since arg will start with ^
...
def main():
    ...
    contributors_hmap = {}
    extensions = [".go", ".py", ".js", ".yaml", ".md", ".sol"]
    weeks = 50
    ...
    for dirpath, filepath in all_dirs_and_files:
        list_of_contributors = get_contributors_for_file(
            dirpath, filepath, extensions, weeks)
